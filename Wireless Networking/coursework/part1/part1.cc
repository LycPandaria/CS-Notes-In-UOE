...
void experiment(int seed, double distance, bool aarf, bool rayleigh)
{
  ...
  if(random < 50){
	  ...
	  Ptr<Ipv4> ipv4Server = wifiStaNodes.Get(0)->GetObject<Ipv4> ();
	  ...
	  onoff.SetConstantRate(dataRate, (uint32_t)1024); //set the onoff client application to CBR mode
	  AddressValue remoteAddress (InetSocketAddress (ipv4AddrServer, 8000)); //specify address and port of the Station as the destination for on-off application's packets
	  onoff.SetAttribute ("Remote", remoteAddress);
	  
	  apps.Add(onoff.Install(wifiApNode.Get(0)));
	  // create and add packet sink app
	  PacketSinkHelper staSink("ns3::UdpSocketFactory", InetSocketAddress (ipv4AddrServer, 8000)); //create packet sink on AP node with address and port that on-off app is sending to
	  apps.Add(staSink.Install(wifiStaNodes.Get(0)));
